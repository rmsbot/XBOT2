...
if (text.includes("!sholat")){
  ...
  axios.get(`https://mhankbarbar.herokuapp.com/api/jadwalshalat?daerah=${teks}&apiKey=LEUJU9ybLwAHbLwnGShv`).then ((res) =>{
...
ᚔᚔᚔᚔᚔᚌᚌᚌᚌᚔᚔᚔᚔᚔ
  ...
  ᚔᚔᚔᚔᚔᚌᚌᚌᚌᚔᚔᚔᚔᚔ
   ...
   {
    var items = ["cewek selebgram", "cewek desa cantik", "cewek cans", "cewek bugil", "cewek sexy", "cewek montok", "cewe cantik", "hijab sexy", "remaja cantik", "remaja montok", "artis bokep"];
...
if (text.includes("!wp cowok"))
   {
    var items = ["wallpaper selebgram cowok", "wallpaper cowo ganteng", "foto wallpaper cogan", "wallpaper cowok indo ganteng", "wallpaper artis cowok ganteng", "wallpaper cogan 2020"];
    var cewe = items[Math.floor(Math.random() * items.length)];
    ...
    axios.get(url)
      .then((result) => {
        ...
        .then(
            (response) => {
    conn.sendMessage(id, '[ WAIT ]❗ Sedang mencari foto wallpaper cowok⏳ silahkan tunggu sebentar', MessageType.text)
	var buf = Buffer.from(response, 'base64'); // Ta-da	
              conn.sendMessage(
            id,
              buf,MessageType.image)
...
if (text.includes("!wp anime"))
   {
    var items = ["wallpaper anime hd", "wallpaper anime keren", "wallpaper anime 2020", "wallpaper anime keren hd"];
    var cewe = items[Math.floor(Math.random() * items.length)];
    ...
    axios.get(url)
      .then((result) => {
        var b = JSON.parse(JSON.stringify(result.data));
        var cewek =  b[Math.floor(Math.random() * b.length)];
        imageToBase64(animek) // Path to the image
        .then(
            (response) => {
    conn.sendMessage(id, '[ WAIT ]❗ Sedang mencari foto wallpaper anime⏳ silahkan tunggu sebentar', MessageType.text)
	var buf = Buffer.from(response, 'base64'); // Ta-da	
              conn.sendMessage(
            id,
              buf,MessageType.image)
  ...
  conn.sendMessage(id, 'Oke Gan Foto Cogannya Ditemukan:v', MessageType.text)
  conn.sendMessage(id, 'Otw Send Foto', MessageTypr.text)
