const BotName = 'BlackXShark_BOT'; // Nama Bot Whatsapp
const instagramlu = 'https://www.instagram.com/itsmeiky'; // Nama Instagramlu cok
...
const grupch3 = 'https://www.instagram.com/itsmeiky'; // OFFICIAL GRUP LU 3
//
...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] BlackSharkXBOTT Ready Scan Now!`);
});
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by itsmeikyxsec`))
...
axios.get(`http://api.fdci.se/sosmed/insta.php?url=${teks}`).then((res) => {
...
conn.sendMessage(id, '*_Powered By ItsmeikyXSec_*', MessageType.text);
         });
   }
